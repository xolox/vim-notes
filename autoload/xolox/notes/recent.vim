...
function! xolox#notes#recent#show(bang, title_filter) " {{{1
  call xolox#notes#misc#msg#info("notes.vim %s: Generating overview of recent notes ..", g:xolox#notes#version)
  ...
  call xolox#notes#misc#buffer#prepare({
        \ 'name': bufname,
        \ 'path': xolox#notes#misc#path#merge($HOME, bufname)})
  ...
  " And we're done!
  call xolox#notes#misc#timer#stop("notes.vim %s: Generated %s in %s.", g:xolox#notes#version, bufname, starttime)
endfunction

function! xolox#notes#recent#track() " {{{1
  let fname = expand('%:p')
  let indexfile = expand(g:notes_recentindex)
  call xolox#notes#misc#msg#debug("notes.vim %s: Recording '%s' as most recent note in %s ..", g:xolox#notes#version, fname, indexfile)
  if writefile([fname], indexfile) == -1
    call xolox#notes#misc#msg#warn("notes.vim %s: Failed to record most recent note in %s!", g:xolox#notes#version, indexfile)
  endif
endfunction
...
function! xolox#notes#recent#edit(bang) " {{{1
  ...
  let indexfile = expand(g:notes_recentindex)
  call xolox#notes#misc#msg#debug("notes.vim %s: Recalling most recent note from %s ..", g:xolox#notes#version, indexfile)
  try
    ...
    call xolox#notes#misc#msg#warn("notes.vim %s: Failed to recall most recent note from %s: %s", g:xolox#notes#version, indexfile, v:exception)
    return
  endtry
  call xolox#notes#misc#msg#info("notes.vim %s: Editing most recent note '%s' ..", g:xolox#notes#version, fname)
